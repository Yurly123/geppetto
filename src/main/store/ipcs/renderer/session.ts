...
export const sessionPreload = {
    saveSession(name: string, messages: Messages) {
        return ipcRenderer.invoke(mainChannel.SAVE_SESSION, name, messages)
    },
    loadSession(name: string): Promise<Messages> {
        return ipcRenderer.invoke(mainChannel.LOAD_SESSION, name)
    },
    deleteSession(name: string) {
        return ipcRenderer.invoke(mainChannel.DELETE_SESSION, name)
    },
    createSession(name: string) {
        return ipcRenderer.invoke(mainChannel.CREATE_SESSION, name)
    },

    setCurrentSession(name: string) {
        return ipcRenderer.invoke(mainChannel.SET_CURRENT_SESSION, name)
    },
